...
int menuinicio(int codigo) { //le pasamos el codigo de usuario
	...
	else { //menu que saldra a los usuaris que ya se han registardo
		printf("1-Ver Perfil\n");
...
void main() {
	//system("CHCP 1252");//Codigo para que se puedan imprimir caracteres especiales, como letras con tildes y ñ

	//system("COLOR F0");//Codigo para cambiar el color del fondo (1er digito) y de las letras (2o digito) (0 = Negro, 1 = Azul, 2 = Verde, 3 = Aqua, 4 = Rojo, 5 = Purpura, 6 = Amarillo, 7 = Blanco, 8 =Gris, 9 = Azul Claro, A = Luz Verde, B = Aqua Luz, C = Rojo, D = Luz Violeta, E = = Amarillo Claro, F = Blanco Brillante) 
	...
	num_usuarios = fread(usuarios, sizeof(struct usuario_estructura), MAX_USUARIOS, f_usuarios); //devuelve el numero de usuarios que hay hasta el momento, n-1, por lo que el nuevo usuario sera el numero n.
	...
	while (num_obras < MAX_OBRAS) { //solo copiamos al vector el numero de obras que puede abarcar
		...
		if (l == 2) {
			num_obras++; // solo sumamos al numero de obras si l=2, porque si no las lineas del fichero que no estan escritas tambien las sumara al numero de obras.
		}
	...
	while (cont) { //cuando el booleano se cambie a 0 (false) , se cierra el programa
		...
		while (fin == 0) {
			...
			case 2: //INICIO DE SESION
				login(usuarios, num_usuarios, &cod_usuario, cartelera, num_obras, &fin, &cont); //el valor del codigo de usuario lo va a modificar la funcion  por ello, puntero
...
void registro(struct usuario_estructura usuarios_completo[], int *n) {
	...
	usuarios_completo[*n].num_entradas = 0; //el numero de entradas  compradas cuando te regitras es 0 siempre
											...
											//Usuario
	printf("Introducir nombre de usuario (maximo 25 caracteres):\n");
	aux_1 = 0;
	do {
		getchar();
		scanf("%s", usuarios_completo[*n].nom_usuario);
		for (k = 0; k < *n; k++) { //comparamos el el nombre de usuario no esta ya cogido por otra persona
	...
	//Contraseña
	do {
		printf("Introducir contraseña (maximo 25 caracteres):\n");
		do {
			getchar();
			scanf("%s", usuarios_completo[*n].contrasena);

			if (strlen(usuarios_completo[*n].contrasena) < 4) {
				printf("Su contraseña es muy insegura, por favor introduzca mas de 3 caracteres\n");
			}
...
void login(struct usuario_estructura usuarios_completo[], int n, int *codigo, struct teatro cartelera[], int num_obras, int *fin, bool *cont) {
	...
	while (intentos < 5) {
		...
		intentos++;
		if (intentos == 4) {
			printf("Numero de intentos agotados. Saliendo del programa...\n"); //como el programa se acaba si te equivocas de contraseña, hay que pasar todas las variables a la funcion exitProg, que al estar dentro de otra funcion, hay que pasarle tambien a la funcion login todo las variables
			exitProg(usuarios_completo, fin, cont, n, cartelera, num_obras);  //no ponemos & delante de fin ni del booleano porque ya estamos pasando por refencia porque estamos dentro de otra funcion a la que ya le hemos hecho el paso por referenca de tales variables
		}
...
void seeProfile(struct usuario_estructura usuarios_completo[], int codigo) {
	...
	printf("Codigo Usuario:  %d\n", codigo); // el codigo que ha modificado la funcionlogin y que ya tien el valor de la posicion del usuario en el vector de estructuras
...
void comprarObra(struct usuario_estructura usuarios_completo[], int codigo, struct teatro cartelera[], int num_obras) {
	...
	printf("Usted ha elegido la obra: %s \n", cartelera[cod_obra - 1].obra); // restamos 1 porque estan almacenadas desde el 0 pero para que en el menu quede bonito, le hemos sumado 1.
	...
	printf("-------------------------------- \n");
	sleep(1); //para que antes de que borre consola, espere unos segundos para poder ver el mensaje anterior
